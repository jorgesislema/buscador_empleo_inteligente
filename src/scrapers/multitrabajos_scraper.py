...
class MultitrabajosScraper(BaseScraper):
    ...
    def fetch_jobs(self, search_params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        while current_page <= MAX_PAGES_TO_SCRAPE_MULTITRABAJOS:
            ...
            job_cards = soup.select('div.aviso-container, article[data-id-aviso]')
            if not job_cards:
            ...
            for card in job_cards:
                ...
                title_link_element = card.select_one('h2 a, a.titulo-aviso')
                ...
                company_element = card.select_one('h3[data-company-name], span.nombre-empresa')
                ...
                location_element = card.select_one('span.location-text, div.ubicacion span')
                oferta['ubicacion'] = self._safe_get_text(location_element)

                date_element = card.select_one('h4.fecha, span.fecha-publicacion')
                ...
                if oferta['url']:
                    ...
                    if detail_soup:
                        desc_container = detail_soup.select_one('div.aviso_description, section.detalle-aviso')
                        if desc_container:
            ...
            next_page_link_element = soup.select_one('a.nav-pag-arrow-right, li.next a')
